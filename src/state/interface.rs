use std::{borrow::Borrow, hash::{Hash, Hasher}, sync::Mutex};
...
impl Clone for Interface {
  fn clone(&self) -> Self {
    Self { name: self.name.clone(), counts: Mutex::new(self.counts.lock().unwrap().clone()), watching: self.watching.clone() }
  }
}
