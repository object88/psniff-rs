...
fn main() -> Result<()> {
	...
	match &c.command {
		...
		Some(Commands::Run(args)) => {
			...
			#[cfg(feature = "channels-console")]
			let (arp_sender, arp_receiver) = channels_console::instrument!((arp_sender, arp_receiver), label = "packet-queue-arp");

			#[cfg(feature = "channels-console")]
			let (ipv4_tcp_sender, ipv4_tcp_receiver) = channels_console::instrument!((ipv4_tcp_sender, ipv4_tcp_receiver), label = "packet-queue-ipv4-tcp");
			...
			let arp_listener_builder = arp_listener::new()
				.set_receiver(arp_receiver);
